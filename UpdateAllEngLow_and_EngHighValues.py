...
tagPath = "[default]GreenMeadows_Tags"
...
print "Found %d tags ending with '_TM':" % len(matchingTagPaths)
print "-" * 80
print "%-50s %-15s %-15s %-15s" % ("Tag Name", "Old engLow", "Old engHigh", "New engHigh")
print "-" * 80
...
for tagPath in matchingTagPaths:
    try:
        ...
        print "%-50s %-15s %-15s %-15s" % (tagPath, str(engLow), str(oldEngHigh), str(newEngHigh))
        ...
        print "%-50s ERROR: %s" % (tagPath, str(e))

# Confirm before updating
print "\n" + "=" * 80
print "Ready to update %d tags. Updating engHigh to 3600 and engLow to 0..." % len(tagConfigsToUpdate)
print "=" * 80
...
for config in tagConfigsToUpdate:
    try:
        ...
        # Update only the engHigh and engLow properties
        tagConfig['engHigh'] = config["engHigh"]
        tagConfig['engLow'] = config["engLow"]
        ...
        if result and len(result) > 0:
            if str(result[0].getName()) == "Good":
                ...
                errorCount += 1
                print "Failed to update: %s - %s" % (config["tagPath"], str(result[0]))
        else:
            successCount += 1

    except Exception as e:
        errorCount += 1
        print "Error updating %s: %s" % (config["tagPath"], str(e))

# Print summary
print "\n" + "=" * 80
print "Update Complete!"
print "Successfully updated: %d tags" % successCount
print "Errors: %d tags" % errorCount
print "=" * 80